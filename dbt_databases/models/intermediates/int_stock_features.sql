...
INNER JOIN {{ ref('int_returns') }} AS r
    ON tsw.ticker = r.ticker AND tsw.date = r.date
INNER JOIN {{ ref('int_volatility') }} AS v
    ON tsw.ticker = v.ticker AND tsw.date = v.date
INNER JOIN {{ ref('int_volume_analytics') }} AS va
    ON tsw.ticker = va.ticker AND tsw.date = va.date
INNER JOIN {{ ref('int_moving_averages') }} AS m
    ON tsw.ticker = m.ticker AND tsw.date = m.date
INNER JOIN {{ ref('int_ohlc') }} AS o
